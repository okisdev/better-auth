...
To unlink a previously linked provider account, use `unlinkAccount` on the client or the corresponding server API. By default, unlinking the user's last remaining account is blocked to prevent lockout.
