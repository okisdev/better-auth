...
To unlink a previously linked provider account, use `unlinkAccount` on the client or the corresponding server API.

<Callout>
By default, unlinking the user's last remaining account is blocked to prevent lockout. You can override this with `allowUnlinkingAll`.
</Callout>

```ts
// client-side usage
await authClient.unlinkAccount({
  providerId: "google", // required
  // accountId: "provider_account_id", // optional, to target a specific account if multiple exist
})
```
...
await auth.api.unlinkAccount({
  body: {
    providerId: "google",
    // accountId: "provider_account_id", // optional
  },
  headers: // pass headers with authenticated token
});
```

If you want to allow unlinking the last remaining account (not recommended in most cases), enable it in your auth configuration:
...
See more in [Users & Accounts → Account Unlinking](/docs/concepts/users-accounts#account-unlinking).
